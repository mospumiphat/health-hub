  ...
  return (
    <main className="min-h-screen bg-paper flex flex-col">
      ...
      <div className="px-6 pt-14 pb-7 border-b-2 border-ink">
        <p className="font-mono text-xs text-ink-3 uppercase tracking-widest mb-3">{today}</p>
        ...
        <p className="font-mono text-xs text-ink-3 tracking-widest uppercase mt-3">Personal Record Book</p>
      </div>
        ...
        {workoutTypes.map((type) => {
          ...
          return (
            <Link key={type} href={`/workout/${type}`} className="block">
              <div className="px-6 py-6 border-b border-rule flex items-center gap-5 active:bg-surface transition-colors relative">
                <div className={`absolute left-0 top-0 bottom-0 w-[3px] ${accent.bg}`} />
                <span className="font-mono text-sm text-ink-3 w-8 shrink-0 pl-1">{meta.numeral}</span>
                ...
                <span className={`font-mono text-lg ${accent.text}`}>→</span>
              </div>
            </Link>
          );
        })}
      ...
      <div className="px-6 pb-12 pt-6 border-t-2 border-ink flex gap-3">
        <Link href="/history" className="flex-1">
          <button className="w-full bg-ink text-paper font-mono text-xs font-bold tracking-widest uppercase py-4 active:bg-ink-2 transition-colors">
            Record Book
          </button>
        </Link>
        <Link href="/import" className="flex-1">
          <button className="w-full border border-rule text-ink-2 font-mono text-xs font-bold tracking-widest uppercase py-4 active:bg-surface transition-colors">
            Import Data
          </button>
        </Link>
      </div>
